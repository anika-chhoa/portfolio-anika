...
export default function Contact() {
  ...
  // EmailJS Credentials
  const SERVICE_ID = "service_qpeophj"; // EmailJS থেকে পাওয়া ID দিন
  const TEMPLATE_ID = "template_pq07kri"; // EmailJS Template ID দিন
  const PUBLIC_KEY = "xjcdYCPzHDuT2I7H3"; // EmailJS Public Key দিন
        ...
        <div className="bg-surface-variant/30 p-8 sm:p-10 rounded-3xl border border-outline relative z-10">
          <form ref={formRef} onSubmit={sendEmail} className="space-y-6">
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              className="w-full py-5 bg-primary text-white font-black rounded-xl hover:bg-primary-hover transition-all flex items-center justify-center gap-3 text-lg shadow-premium disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
                  Sending...
